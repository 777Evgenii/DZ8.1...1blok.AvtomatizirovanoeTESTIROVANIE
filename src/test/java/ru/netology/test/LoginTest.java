...
public class LoginTest {
    ...
    void shouldInvalidLogin() {
        ...
        var authUser = new DataHelper.AuthUser(
                DataHelper.getRandomUser().getLogin(),
    ...
    void shouldInvalidPass() {
        var loginPage = open("http://localhost:9999", LoginPage.class);
        var authUser = new DataHelper.AuthUser(
                DataHelper.getAuthUser().getLogin(),
    ...
    void shouldPassThreeTimes() {
        var loginPage = open("http://localhost:9999", LoginPage.class);
        var authUserOne = new DataHelper.AuthUser(
                DataHelper.getAuthUser().getLogin(),
                DataHelper.getRandomUser().getPassword()
        );
        ...
        var authUserTwo = new DataHelper.AuthUser(
                DataHelper.getAuthUser().getLogin(),
                DataHelper.getRandomUser().getPassword()
        );
        ...
        var authUserThree = new DataHelper.AuthUser(
                DataHelper.getAuthUser().getLogin(),
                DataHelper.getRandomUser().getPassword()
        );
